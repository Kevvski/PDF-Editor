...
class App {
    constructor() {
        ...
        this.initializeMainNavBar = () => {
            ...
            navElements.forEach((e, i) => {
                e.addEventListener("click", () => {
                    navElements.forEach((en) => {
        ...
        this.loadPdfDocument = async () => {
            ...
            let CLICK_SETPOINT;
            (function (CLICK_SETPOINT) {
                CLICK_SETPOINT[CLICK_SETPOINT["FIRST_CLICK"] = 0] = "FIRST_CLICK";
                CLICK_SETPOINT[CLICK_SETPOINT["SECOND_CLICK"] = 1] = "SECOND_CLICK";
            })(CLICK_SETPOINT || (CLICK_SETPOINT = {}));
            ;
            const pdfViewer = document.getElementById("pdfViewer");
            const changeFileContainer = document.getElementById("changeFileContainer");
            const drawArea = document.getElementById("drawArea");
            let setAreaClicks = CLICK_SETPOINT.FIRST_CLICK;
            const drawAreaEvent = (e) => {
                e.preventDefault();
        ...
        this.savePdfDocument = async () => {
            if (this.pdfDocument !== undefined) {
                ...
                const colorValue = document.getElementById("colorInput").value.split('');
                ...
                const convertPosToPDF = (axis, value) => {
                    let windowValue = 0;
                    let pageValue = 0;
                    const scale = (drawContainerRect.width / pages[0].getWidth());
                    if (axis === "x") {
